...
public class TaskSchedulerController {
	...
	public @ResponseBody ResponseEntity<List<TaskResponse>> getApplications() {
		...
		logger.info("List Applications :Response: %s", taskResponse);
	...
	public @ResponseBody ResponseEntity<List<TaskResponse>> getTasksOfApplication(@PathVariable String appId) {
		logger.info("List Tasks for an Applications :Request: %s ", appId);
		List<TaskResponse> taskResponse = taskSchedulerService.listTasksForApplication(appId);
		logger.info("List Tasks for an Applications :Response: %s ", taskResponse);
		return new ResponseEntity<List<TaskResponse>>(taskResponse, HttpStatus.OK);
	}
	...
	public @ResponseBody ResponseEntity<TaskResponse> getTaskOfApplication(@PathVariable String appId, @PathVariable String taskId) {
		logger.info("List Task of an Applications :Request: %s, %s ", appId, taskId);
		TaskResponse taskResponse = taskSchedulerService.getTaskDetails(appId, taskId);
		logger.info("List Task of an Applications :Response: %s ", taskResponse);
	...
	public @ResponseBody ResponseEntity<TaskResponse> createTaskOfApplication(@PathVariable String appId, @PathVariable String taskId,
		...
		logger.info("Create Task of an Applications :Request: %s, %s ", appId, taskId);
		TaskResponse taskResponse = taskSchedulerService.createTaskForApplication(appId, taskId, request);
		logger.info("Create Task of an Applications :Response: %s ", taskResponse);
	...
	public @ResponseBody ResponseEntity<TaskResponse> deleteTaskOfApplication(@PathVariable String appId, @PathVariable String taskId,
			@RequestBody TaskRequest request) {
		logger.info("Delete Task of an Applications :Request: %s, %s ", appId, taskId);
		TaskResponse taskResponse = taskSchedulerService.deleteTaskForApplication(appId, taskId, request);
		logger.info("Delete Task of an Applications :Response: %s ", taskResponse);
		return new ResponseEntity<TaskResponse>(taskResponse, HttpStatus.OK);
	}

	@RequestMapping(value = "/{appId}/{taskId}/start", method = RequestMethod.PUT)
	public @ResponseBody ResponseEntity<TaskResponse> startExecution(@PathVariable String appId, @PathVariable String taskId,
		...
		logger.info("Starting the Task :Request: %s , %s, %s ", appId, taskId, taskRequest);
		TaskResponse taskResponse = taskSchedulerService.startTaskExecution(appId, taskId, taskRequest);
		logger.info("Starting the Task :Response: %s , %s, %s ", appId, taskId, taskRequest);
	...
	@RequestMapping(value = "/{appId}/{taskId}/stop", method = RequestMethod.PUT)
	public ResponseEntity<TaskResponse> stopExecution(@PathVariable String appId, @PathVariable String taskId, @RequestBody TaskRequest taskRequest) {
		logger.info("Stopping the Task :Request: %s , %s, %s ", appId, taskId, taskRequest);
		TaskResponse taskResponse = taskSchedulerService.stopTaskExecution(appId, taskId, taskRequest);
		logger.info("Stopping the Task :Response: %s , %s, %s ", appId, taskId, taskRequest);
